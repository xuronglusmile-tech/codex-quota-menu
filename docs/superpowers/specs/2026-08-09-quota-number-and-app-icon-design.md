...
1. 将额度窗口中每个额度窗口右侧的剩余百分比数字固定显示为黑色。
2. 将状态栏药丸左侧的实时百分比数字固定显示为白色。
3. 为应用包配置一个可被 Finder、启动台和 Dock 识别的 macOS 图标。
4. 保留现有菜单栏中的实时额度百分比和颜色药丸，不用静态应用图标替换实时状态显示。
...
`Text("\\(window.remainingPercent)%")` 显式设置黑色前景色。在
`Sources/CodexQuotaMenu/UI/MenuBarQuotaLabel.swift` 的 `NSAttributedString` 文本属性中，
为状态栏百分比显式使用 `NSColor.white`。只改变两个数字的呈现，不改变额度模型、
计算、阈值或其他文字层级。
